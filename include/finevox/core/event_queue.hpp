 ...
 * Outbox consolidation: keyed by (BlockCoord, EventType)
 */
 ...
 *
 * Events pushed to the outbox are consolidated by BlockCoord before being
 ...
 *
 * Consolidation rules:
 * - NeighborUpdated: Merge face masks (OR together)
...
public:
     ...
     */
    void scheduleTick(BlockCoord pos, int ticksFromNow, TickType type);
     ...
     */
    void cancelScheduledTicks(BlockCoord pos);

    /**
     * @brief Check if a block has any pending scheduled ticks
     */
    [[nodiscard]] bool hasScheduledTick(BlockCoord pos) const;
