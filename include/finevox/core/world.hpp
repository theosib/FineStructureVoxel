...
public:
    ...
    /// @return true if event was queued, false if no scheduler set
    bool breakBlock(BlockCoord pos);
    ...
    /// @return number of events queued (0 if no scheduler set)
    size_t breakBlocks(const std::vector<BlockCoord>& positions);
    ...
    [[nodiscard]] std::vector<ChunkPos> getAffectedSubChunks(BlockCoord blockPos) const;
    ...
    void registerForceLoader(BlockCoord pos, int32_t radius = 0);

    /// Unregister a force-loader
    /// No-op if position wasn't registered
    void unregisterForceLoader(BlockCoord pos);
    ...
    [[nodiscard]] bool isForceLoader(BlockCoord pos) const;

    /// Get all registered force-loaders (for serialization)
    [[nodiscard]] const std::unordered_map<BlockCoord, int32_t>& forceLoaders() const;

    /// Set force-loaders from deserialization
    /// Replaces any existing force-loaders
    void setForceLoaders(std::unordered_map<BlockCoord, int32_t> loaders);
    ...
    /// No-op if light engine is not set
    void processLightingUpdateSync(BlockCoord pos, BlockTypeId oldType, BlockTypeId newType);
    ...
    /// No-op if light engine is not set.
    void enqueueLightingUpdateWithRemesh(BlockCoord pos, BlockTypeId oldType, BlockTypeId newType);
...
private:
    ...
    std::unordered_map<BlockCoord, int32_t> forceLoaders_;

    // Helper to convert block position to column position
    [[nodiscard]] static ColumnPos blockToColumn(BlockCoord pos);
